...
1. Create `.env` from the `.env.example` template with random secrets filled
   in (`.env` is gitignored; skip if it already exists — re-running overwrites
   the passwords):
```bash
cp .env.example .env \
  ...
  && rm -f .env.bak
```
   `DATABASE_URL` follows `${DB_PASS}` automatically. Then edit `.env` and set
   the `*_MAP_API_KEY` placeholders to real keys, and — only if you use
   realtime push — `MQTT_DEVICE_SHARED_SECRET` to the same value the Android
   app embeds (all other MQTT/EMQX credentials were already generated above).

2. Optional, if running via docker:
```bash
...
Provision authn/authz once (idempotent; config persists in `mqtt-data/`),
after `/mqtt/auth` is live:
...
The EMQX credentials are generated by Install step 1 into `.env`. EMQX reads
its bootstrap API key only on a fresh `mqtt-data/`; to regenerate, change the
keys in `.env`, then `docker compose down && rm -rf mqtt-data && docker
compose up -d` and re-run the commands above. Rotating
`MQTT_DEVICE_SHARED_SECRET` requires updating the Android app to match.
